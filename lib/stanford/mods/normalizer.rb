  ...
  module Mods
    ...
    class Normalizer
      ...
      LONE_DATE_XPATH = '//mods:originInfo/mods:dateCreated[1][not(following-sibling::*[1][self::mods:dateCreated])] ' \
                        '| //mods:originInfo/mods:dateIssued[1][not(following-sibling::*[1][self::mods:dateIssued])]'
      ...
      #
      # Nodes that fall under any of these exceptions should not be deleted,
      # even if they have no content.
      #
      # @param  [Nokogiri::XML::Element] node An XML node.
      # @return [Boolean] true if the node contains any of the
      #   exceptional attributes, false otherwise.
      ...
      def exceptional?(node)
        ...
        attributes.each do |key, value|
          ...
          # Note that according to the MODS schema, any other
          # value than 'yes' for these attributes is invalid
      ...
      #
      # @param [Nokogiri::XML::Element]   node   An XML node
      # @return [String] A string composed of the entire contents
      #   of the given node, with substitutions made as described
      #   for {#clean_linefeeds}.
      ...
      def substitute_linefeeds(node)
        ...
        # If we substitute in '&#10;' by itself, Nokogiri
        # interprets that and then prints '&amp;#10;' when
        # printing the document later. This is an ugly way to
        # add linefeed characters in a way that we at least get
        # well-formatted output in the end.
        if node.text?
      ...
      # Given the root of an XML document, replaces linefeed
      # characters inside <tableOfContents>, <abstract> and
      # <note> XML node by &#10;
      ...
      # </p> is removed
      # \r\n is replaced by &#10;
      # Any tags not listed above are removed. MODS 3.5 does
      # not allow for anything other than text inside these
      # three nodes.
      #
      # @param [Nokogiri::XML::NodeSet] node_list All
      #   <tableOfContents>, <abstract> and <node> elements.
      # @return [Void] This method doesn't return anything,
      #   but introduces UTF-8 linefeed characters in place,
      #   as described above.
      ...
      #
      # @param [String]     input   The text of an XML node.
      # @return [String] The cleaned string, as described.
      #   Returns nil if the input is nil, or if the input
      #   is an empty string.
      def clean_text(input)
        return nil unless !input.nil? && input != ''
      ...
      # @param [Nokogiri::XML::Element]   node An XML node.
      # @return [Void] This method doesn't return anything,
      #   but modifies the XML tree starting at the given
      #   node.
      ...
      # Removes empty nodes from an XML tree. See
      # {#exceptional?} for nodes that are kept even if empty.
      #
      # @param  [Nokogiri::XML::Element] node An XML node.
      # @return [Void] This method doesn't return anything,
      #   but modifies the XML tree starting at the given
      ...
      # @param  [Nokogiri::XML::Element]  node An XML node.
      # @return [Void] This method doesn't return anything,
      #   but modifies the entire XML tree starting at the
      #   given node, removing leading and trailing spaces
      #   from all text. If the input is nil, an exception
      #   will be raised.
      def trim_text(node)
        children = node.children
      ...
      # Sometimes there are spurious decimal digits within
      # the date fields. This method removes any trailing
      # decimal points within <dateCreated> and <dateIssued>.
      #
      # @param [Nokogiri::XML::NodeSet] nodes A set of all
      #   affected <dateCreated> and <dateIssued> elements.
      # @return [Void] The given document is modified in
      #   place.
